...
<template>
    <div class="cov-vue-date">
        <div class="datepickbox">
            <input type="text" title="input date" class="cov-datepicker" readonly="readonly" :placeholder="option.placeholder" v-model="date" :required="required" @click="showCheck" @foucus="showCheck" :style="option.inputStyle ? option.inputStyle : {}" />
        </div>
...
export default {
    ...
    methods: {
        ...
        showDay(time) {
            ...
            this.showOne("day");
            this.checked.year = moment(this.checked.currentMoment).format(
                "YYYY"
            );
            this.checked.month = moment(this.checked.currentMoment).format(
                "MM"
            );
            ...
            for (
                let i = 0;
                i < firstDay - (this.option.SundayFirst ? 0 : 1);
                i++
            ) {
                let passiveDay = {
        ...
        limitWeekDay(limit, days) {
            days.map(day => {
                if (
                    limit.available.indexOf(
                        Math.floor(day.moment.format("d"))
                    ) === -1
                ) {
        ...
        checkDay(obj) {
            ...
            if (this.option.type === "day" || this.option.type === "min") {
                ...
                let ctime =
                    this.checked.year + "-" + this.checked.month + "-" + day;
                if (obj.checked === true) {
                    obj.checked = false;
        ...
        picked() {
            if (this.option.type === "day" || this.option.type === "min") {
                ...
                this.date = moment(this.checked.currentMoment).format(
                    this.option.format
                );
            } else {
        ...
        dismiss(evt) {
            if (evt.target.className === "datepicker-overlay") {
                if (
                    this.option.dismissible === undefined ||
        ...
        shiftActTime() {
            ...
            this.$nextTick(() => {
                ...
                document.querySelector(".hour-box").scrollTop =
                    (document.querySelector(".hour-item.active").offsetTop ||
                        0) - 250;
                document.querySelector(".min-box").scrollTop =
                    (document.querySelector(".min-item.active").offsetTop ||
                        0) - 250;
            });
        }
    },
